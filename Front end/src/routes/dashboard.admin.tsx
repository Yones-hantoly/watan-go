...
import { Car, ShieldCheck, Activity, MapPin, Navigation, UserCheck, X } from "lucide-react";
...
export const Route = createFileRoute("/dashboard/admin")({
  component: () => (
    <DashboardShell expectedRole="admin">{(user) => <AdminDashboard user={user} />}</DashboardShell>
  ),
});
    ...
    <AdminWorkspace user={user}>
      <section className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard
          label="رحلات اليوم"
          value={String(
            rideOrders.filter((r) => r.createdAt.startsWith(new Date().toISOString().slice(0, 10)))
              .length,
          )}
          accent="primary"
        />
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="card-elevated p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-lg font-bold">نشاط مباشر</h2>
          </div>
        ...
        <div className="card-elevated p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-lg font-bold">تنبيهات الأمان</h2>
          </div>
          <ul className="mt-3 space-y-2 text-sm">
            <li className="flex items-center justify-between">
              <span>محاولات دخول مشبوهة</span>
              <span className="font-mono text-xs font-bold text-amber">٢</span>
            </li>
            <li className="flex items-center justify-between">
              <span>حسابات بانتظار التوثيق</span>
              <span className="font-mono text-xs font-bold text-cyan">٥</span>
            </li>
            <li className="flex items-center justify-between">
              <span>بلاغات نشطة</span>
              <span className="font-mono text-xs font-bold text-primary">١</span>
            </li>
          </ul>
        </div>
      </section>
